...
- **A genuine watchdog reset names itself on the next boot — with one field-verified caveat**: the reset itself is silent, but `core1_main()` captures `machine.reset_cause()` once (via `SystemInformation.get_reset_cause()`, a stable short label — `wdt` for the WDT_RESET value, `poweron` for the PWRON_RESET value, else `unknown`; the v1.28 rp2 port reports no finer cause, and field-verified on the flashed build a `machine.reset()` reboot reports the WDT_RESET value, so `wdt` covers both a deliberate reset and a watchdog expiry) and the `system_startup_completed` log carries it as `reset_cause` (in both the full and the bounded fallback form), so a genuine watchdog reset is diagnosable from the broker log as a `wdt` boot with no preceding `rebooting: true` acknowledgement (Core 0 publishes it immediately before a commanded reboot) instead of being only the absence of a shutdown log. The main.py recovery-boundary reset publishes no acknowledgement, so it reads the same as a watchdog fire — a known, accepted limit (a persistent commanded-reboot marker was considered and deliberately not built); the serial `[FATAL]` line remains the recovery boundary's own trace. The get-details `machine` section reports the same field on demand, and a dropped MQTT session is attributed the same way: `Mqtt` records a stable bounded `last_disconnect_reason` (`wifi_lost` / `ping_timeout` / `publish_failure` / `check_msg_failure`) carried in the network snapshot and exposed in the get-details `communications` section.
