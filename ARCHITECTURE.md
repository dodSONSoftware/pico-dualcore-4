...
- Raspberry Pi Pico W: `RPI_PICO_W with RP2040` → canonical type `pico_w`, preferred reserve 60 KiB, hard floor 48 KiB
...
The acknowledgement frames are shape-checked the same way, one by one: CONNACK must be opcode 0x20 with its fixed 2-byte body, SUBACK must declare exactly the 3-byte body of this client's single-topic subscription (2-byte packet id + 1 return code) — read **before** the body, so a declared length that is too long leaves no stray byte in the stream and one that is too short over-consumes none of the next packet's bytes — with the return code gated to the grants at or below the requested QoS 1 (0x00/0x01) or 0x80 (failure; a QoS 2 grant or a reserved code is a protocol violation, not a grant), PUBACK must be exactly its 2-byte packet id, and PINGRESP must carry zero remaining length. A frame that does not match its declared shape drops the connection via `_abort_corrupt_inbound`: accepting it would desynchronize the stream, so the failure is a dropped connection (recovery reconnects), not a reported protocol error.

The opcode gate makes the inbound frame set complete against what a conforming broker can send this client: PUBLISH (QoS 0/1) is consumed in full and delivered to the callback, PINGRESP is consumed in place, and PUBACK and SUBACK are returned **body unconsumed** to the `publish()`/`subscribe()` wait loop that consumes them positionally. Every other control frame — a broker DISCONNECT, an UNSUBACK (nothing is ever unsubscribed, so none can legitimately arrive), or a reserved or corrupt opcode — is consumed by no wait loop, and returning after the opcode byte would leave the frame's remaining length and body in the stream for the next `read(1)` to misparse as a new packet (a leftover byte in 0x30–0x37 misparses as a PUBLISH and can swallow the following real frame). `wait_msg()` therefore drops the connection via `_abort_corrupt_inbound` — the DISCONNECT named as such (`Broker DISCONNECT`) — and the abort's `MQTTException` is an expected transport failure, so the drop drives the ordinary recovery reconnect rather than a controlled reset.

The 20 KiB value is derived from the protocol, not arbitrary: the largest spec-valid inbound frame is a `write-config` command carrying the worst-case valid configuration (every byte-bounded identity at 4-byte code points, which serialize 3× in the escaped form, the **character**-bounded 128-character `command_id`/`target`, the 15-character broker address (the longest valid dotted quad), all 16 devices, all lists at their bounds) — 16,329 bytes in the conservative wire form (ASCII-escaped, default separators). The serialized-size invariant test in `tests/test_config.py` pins both halves of the config boundary: the worst-case read-config **response** (envelope splice included) at or under the 16 KiB outbound ceiling, and the worst-case write-config **command** under the 20 KiB inbound ceiling — a ceiling set below the largest valid command (16,384 once was) would drop a spec-valid command at the wire layer, never letting validation reject or accept it.
...
- **Only `get-details` crosses.** Core 1 no longer acts as the generic fallback for arbitrary command names: a name outside the registry is answered on Core 0 and never crosses to Core 1.
...
- **Configuration** — there is no persistent `needs_reboot` flag or file. A reboot that follows a `reboot_required` configuration change needs no special branch: the reset clears the RAM-only debounce cache and the in-boot `reboot_required` flag, and startup reloads the authoritative persisted `config.json` with `reboot_required == false`.
...
- **The handshake is bounded the same way**: `Mqtt.connect()` calls `MQTTClient.connect(timeout=mqtt_broker_response_timeout_sec)`, which installs the finite socket timeout that the CONNACK wait runs under. Before the handshake, the broker address is resolved with `getaddrinfo()` filtered to `AF_INET`/`SOCK_STREAM` — the profile the default socket constructs — so the lookup cannot hand `connect()` an unusable first record (an IPv6 or datagram result) even when a usable one follows. The config boundary requires a numeric IPv4 dotted quad, so the lookup parses without a DNS query: that matters because the lookup sits outside the socket timeout — the Core 0 servicing feed fires before it, as before every subsequent socket operation of the handshake (see Core 0 hardware watchdog), but not during it — and a hostname's query would be bounded only by lwIP's own DNS retry logic, which can stretch the un-fed stretch past the 8 s watchdog budget and reset the board instead of failing the attempt into the reconnect backoff. A lookup with no usable record fails as `OSError`, the ordinary transport failure, into the reconnect backoff. `mqtt_broker_response_timeout_sec` is operationally bounded at the config boundary (`config.MAX_MQTT_BROKER_RESPONSE_TIMEOUT_SEC`, 5 s) so every wait it sizes — handshake, PUBACK, `check_msg` completion, UTC request deadline — stays under the Core 0 hardware watchdog budget (see Core 0 hardware watchdog). `subscribe()` does not install its own timeout — it relies on the one `connect()` left in place — so that same finite timeout carries across both SUBACK waits. A broker that accepts the TCP connection and then goes silent fails the attempt within the timeout (the reconnect backoff then retries) instead of wedging Core 0. Once both subscriptions succeed, `Mqtt.connect()` restores normal blocking mode; every later operation (PUBACK, PINGRESP) installs and restores its own timeout.
...
- **exception taxonomy at the device boundary**: only `OSError` is a recoverable device failure. Every driver normalizes its hardware/I2C failures to it (a bus drop, a sensor timeout, a missing chip — and the ds18b20 driver additionally normalizes the MicroPython `ds18x20` module's bare-`Exception` scratchpad CRC raise, the module's one hardware failure raised outside `OSError`; see the DS18B20 device section), so each recovery path — driver construction, boot initialization, the normal read, the runtime reinitialization, and the boot-failure late-retry pass — catches `OSError` beside the `MemoryError` re-raise, and those failures stay logged, retried, and reinitializable sensor conditions. Anything else is a firmware defect that reinitializing cannot repair, and it escapes `DeviceManager` to Core 1's worker boundary (a dead worker is recovered by Core 0's heartbeat watchdog) instead of being reclassified as a failed sensor and retried indefinitely into the same deterministic fault: the manager's own `TypeError` for a `read()` that is not a non-empty JSON-safe dict, a `ValueError` from a driver that reached its validator despite the config-boundary validation (a `DeviceValidationError` — a driver `initialize()` that reaches the validator means the validation was bypassed), a `RuntimeError`/`ArithmeticError` driver-state defect, and the I2C bus factory's same-bus-conflict `ValueError`
- **boot-failure late recovery**: a device that exhausts its boot initialization attempts is not permanent — the Core 1 run loop retries it at a slow steady-state interval until it initializes. The interval reuses `device_initialization_retry_delay_ms` with a 1 s floor (the key is legal at 0; 0 would probe the sensor on every 20 ms loop tick). Each pass makes exactly **one** fresh `initialize()` attempt per failed device (in configuration order): the slow outer interval is the retry policy, so the pass re-runs no attempts × delay inner loop and a dead sensor costs at most one bounded driver call — it can never hold the read pass that paces every working sensor's telemetry. On success the device is promoted to a fresh READY `ManagedDevice` in the normal active lifecycle (cumulative `initialization_attempts_used`, zero read-failure state) and leaves the failure record; a failed attempt updates the record (cumulative attempts, last error) and the next pass retries it. Failure logging follows the reinit-suppression pattern (first steady-state retry failure warns, repeats suppressed until recovery, a later independent failure warns again), and a recovery logs an INFO line. The loop's gate is a single bool check that costs nothing once no boot-failed devices remain; health `devices_active` and the get-details `device_status` section reflect a recovered device through the existing snapshot walks

The supported `device_type` registry (`device_factory._DEVICE_REGISTRY`) maps each type to its validation package and its validator/allowed-config-keys attributes, resolved by import at first use — a type a board does not configure is never imported. Five types are registered:
...
- `ds18b20` — the third hardware (1-Wire) device: an Analog Devices DS18B20 digital thermometer.
- `sht35` — the fourth hardware (I2C) device: a Sensirion SHT35-DIS temperature/relative-humidity sensor.
- `yl69_fc28` — the fifth hardware device (the first ADC device): a YL-69 / FC-28 soil-conductivity probe (LM393 comparator module), reporting relative calibrated moisture.
...
- `BME280Device` is the `Device` adapter that applies the application-layer policy the spec keeps out of the fundamental compensation: user offsets (`offsets.{temperature_c, humidity_percent, pressure_pascal}`) applied after Bosch compensation, and a derived `altitude_m` from the offset-adjusted pressure and `sea_level_pressure_pa`. The derivation is guarded to its domain: `altitude_m` is `null` when the offset-adjusted pressure is non-positive — a schema-legal `pressure_pascal` offset (bound ±200,000 Pa) can drive it there, where the barometric formula's fractional power is undefined (`ValueError` on MicroPython, a non-JSON-safe complex on the host), and an unguarded derivation would escape the driver's `OSError`-only operational domain into a reboot loop via the worker boundary; the pressure itself stays reported, so a bad offset remains visible in the telemetry. Telemetry is `{temperature_c, pressure_pa, humidity_percent, altitude_m}` (pressure stays in Pa; the presentation layer converts to hPa).
...
**I2C bus ownership.** Core 1 owns its I2C buses (see Ownership invariants); a driver never creates one. The bus is configured **per device** (`i2c_bus`, optional `i2c_sda_pin`/`i2c_scl_pin`/`i2c_freq_hz`), and each device's pins are checked to be **routable** to the selected controller: on the RP2 the I2C SDA/SCL are fixed by the GPIO mux table (each controller's SDA and SCL occupy one GPIO group of four — SDA even / SCL odd within the group), and the RP2 port enforces exactly this mapping in the `machine.I2C(...)` constructor. The routable set is the mux group further intersected with the pins the board actually exposes: the Pico W / Pico 2 W reserve four of the silicon's 30 GPIOs for the CYW43 — GP23 (wireless power-on), GP24 (wireless SPI data/IRQ), GP25 (wireless SPI chip-select) and GP29 (wireless SPI clock / VSYS ADC) — and the boards bring out GP0–22 and GP26–28, so a reserved pin is a configuration error even though the silicon mux routes it (the single source of this board rule is `devices/rp2_pins.py`). A pin that cannot route to the chosen `i2c_bus` (e.g. I2C0's 4/5 pair offered to `i2c_bus` 1) is a deterministic configuration error, rejected at the config boundary by the shared pure validator in `devices/rp2_i2c.py` (used by both I2C sensors, and host-importable like the per-sensor validators) instead of surfacing as an operational device failure at construction. On the RP2 port `machine.I2C(bus)` configures the physical controller itself — MicroPython keeps one static object per controller, and construction sets that controller's pins and clock — so the bus identity is the **controller**, and two devices on the same `i2c_bus` must carry identical effective `(sda, scl, freq)` settings (an absent `i2c_freq_hz` resolves to the device type's default; absent pins stay distinct from explicit ones, so a conflict error is answered by making the settings identical). `validate_config` enforces that cross-device rule — a same-bus conflict is a deterministic configuration error rejected at the config boundary (startup and write-config share the path, and the error names both devices) instead of reaching hardware, where the second construction would silently reconfigure the controller the first device already runs on and surface as flaky sensor reads that look like a failing device. `core1_main` hands `DeviceManager` a bus factory that builds one `machine.I2C` per controller and caches it: a same-bus request with the identical settings returns the shared object, and a conflicting same-bus request raises (defense in depth against bypassed validation — the `ValueError` is a configuration/programming error, so it escapes the device manager's `OSError`-only recovery domain to Core 1's worker boundary (the dead worker is recovered by Core 0's heartbeat watchdog) instead of being recorded as a failed device that the retry machinery would re-probe and hide, or destabilizing the first device). The factory imports `machine` lazily (inside the closure) so a config with no I2C device allocates no bus or pins. `create_device` resolves a device's bus through the factory (the factory is injected, so `device_factory`/`device_manager` stay host-importable). The bus is created at driver construction (peripheral + pins only); the sensor protocol — and therefore the failure that a missing/miswired sensor surfaces — runs in `initialize()`, which the retry/reinit machinery wraps, so an absent BME280 degrades to a failed device rather than a boot halt.
...
**Bus sharing.** The shipped `config.json` places the `ltr390` on the same bus/pins as the `bme280` (bus 0, GPIO 0/1): identical effective `(sda, scl, freq)` on one controller, so the lazy bus factory hands both sensors the single `machine.I2C` for bus 0 (the addresses do not collide: 0x53 vs 0x76/0x77). A same-bus device with different pin or clock settings is rejected by `validate_config` before it can reconfigure the controller under the other sensor.

**Physical-sensor uniqueness.** The same-bus rule above is about *controller settings*; a distinct rule is about the *chip on the wire* — a logical `id` is unique, but two device definitions can still name the same directly-attached I2C chip, which the settings rule cannot see. `validate_config` enforces that cross-device boundary (after per-device validation): at most **one `ltr390` per `i2c_bus`**, because its address is fixed at 0x53 and a second on the bus would target the same sensor; and more than **one `bme280` per `i2c_bus` only when each names exactly one distinct `i2c_address_candidates` address**. The reason is the probe semantics: the BME280 driver binds the *first responder* in its candidate list, so a multi-address list — including the two-address default `(118, 119)`, shared from `DEFAULT_I2C_ADDRESS_CANDIDATES` in `devices/bme280/validation.py` — is ambiguous while a neighbor is present (an omitted candidate list resolves to that same default and is ambiguous too), whereas `[118]` and `[119]` each identify one chip. A single `bme280` on a bus keeps its full candidate-list behavior unchanged. Both violations are deterministic configuration errors (naming the bus and the device ids), not runtime conditions — no address scanning or probing at validation time.

### DS18B20 device

The `ds18b20` device is the first 1-Wire sensor, split the same way: a low-level protocol class and a `Device` adapter.

- `DS18B20` (`devices/ds18b20/ds18b20_device.py`) owns the protocol: the bus scan with **ROM identification** (the configured 64-bit ROM must be present among the scan results, matched by value — a scan-list position is never an identity, because a rescan after a fault can reorder or drop entries and the ROM is the only physical identifier that survives it), and the **two-stage read**: `convert_temp()` starts the conversion, the configured conversion window (default 750 ms) elapses as a **single bounded sleep with the data pin left configured but undriven** — the DQ line is open-drain, so an idle pin is high-Z and a multi-drop line stays free for any other master while the sensor converts on its VDD supply (the documented MicroPython `ds18x20` API exposes no pin release, and none is needed) — and only then does `read_temp()` return a fresh value. The two-stage sequence is the freshness boundary: the scratchpad holds the *previous* value (85 °C after a power cycle) until the conversion completes, so the wait is part of the protocol, not a performance detail. The read is range-gated to the documented −55…+125 °C — a value outside it is a corrupted or absent reading and fails as an operational `OSError` — and **85.0 °C is deliberately never rejected**: it is the power-on scratchpad value *and* a valid real temperature, so filtering it would discard a real measurement (the correct software is convert → wait → read, not an 85.0 filter). It returns one Celsius value (the driver stays in °C; the presentation layer converts units). **Bus-failure normalization**: the injected bus object's failure domain is operational, so `read()` normalizes every non-`MemoryError` raise from it to `OSError` — including the MicroPython `ds18x20` module's bare-`Exception` scratchpad CRC raise, a bus-integrity condition (bit corruption in transit: a stale scratchpad is still CRC-valid, so a CRC failure is not a stale read). Without the normalization the CRC error escapes the driver's boundary and kills the Core 1 worker (recovered only by the 30 s heartbeat-stale reset) over one flaky read; with it, the failure feeds the normal read-failure/reinitialization path (counted, warned once, reinit at `device_read_failure_threshold`) like any other sensor condition.
- `DS18B20Device` is the `Device` adapter that applies the application-layer policy the protocol keeps out: the user offset (`offsets.temperature_c`) applied after the read. Telemetry is `{temperature_c}`.
- Both classes stay host-importable: `DS18B20` receives the injected `ds18x20.DS18X20` bus object and `DS18B20Device` imports no `machine`/`onewire`/`ds18x20` API, so the protocol logic is unit-testable against a fake bus without hardware.

**The ROM is the config key.** `rom` is the sensor's 64-bit ROM as a 16-hex-character string (case-insensitive, canonical lowercase); its first byte must be the DS18B20 family code `0x28` — a different family (the DS18S20's `0x10` decodes its temperature register differently) is a deterministic configuration error at the config boundary, not a runtime miss. `pin` is the 1-Wire data line: 1-Wire is software-timed, so there is no mux routing constraint the way I2C's SDA/SCL groups have — any of the board's externally exposed GPIOs is legal. The four wireless-reserved pins (GP23/24/25/29, the CYW43 allocation on the Pico W / Pico 2 W) are not: they are not brought out for external use and are driven by the radio, and because config is remotely writable and applied at reboot, a data line on one of them is a "valid" configuration that loses network connectivity — a deterministic configuration error rejected at the config boundary by the same shared board rule (`devices/rp2_pins.py`) the I2C routing check applies. `conversion_ms` is bounded **750–1000 ms**: the power-on resolution is 12 bit with a 750 ms maximum conversion time, and the driver never configures or verifies the sensor's resolution — a shorter wait would race the scratchpad and read the previous value (the indistinguishable 85 °C after a power cycle). The tight ceiling is sensor-derived — no resolution's conversion window comes close to it, and the slack over the 750 ms floor is clone/timing margin, not protocol headroom — and the liveness budget enforces it from above: the wait is one uninterrupted sleep in the driver's `read()` that does not refresh Core 1's liveness stamp, so a value anywhere near Core 0's 30 s Core 1 staleness timeout (the former 60000 ceiling permitted it) would be schema-legal yet reset the device after every read, including after a remotely written config.

**1-Wire bus ownership.** Core 1 owns its 1-Wire buses (see Ownership invariants); a driver never creates one. `core1_main` hands `DeviceManager` a bus factory that builds one `ds18x20.DS18X20(onewire.OneWire(Pin(pin)))` per data pin and caches it: `machine`/`onewire`/`ds18x20` are imported lazily inside the closure, so a config with no 1-Wire device allocates no bus and imports nothing. Unlike the I2C factory there is **no same-bus conflict raise**: a 1-Wire pin carries no reconfigurable setting (no second signal, no clock), so a second device on the same pin simply shares the bus — two `ds18b20` entries on one pin with distinct ROMs is a multidrop bus sharing one `DS18X20` (the convert is unaddressed, so it starts every sensor on the pin and the conversions run in parallel; each device's wait leaves the pin configured but undriven — open-drain idle is high-Z — so the shared data line stays free while they run). The cross-bus rule instead runs at the config boundary: a `ds18b20` `pin` may not equal any I2C device's **effective** `i2c_sda_pin`/`i2c_scl_pin` — one GPIO cannot carry two bus protocols, and the overlap would surface as flaky reads on both sensors that look like failing hardware. An I2C pin left out of the config is not invisible to this check: at runtime `machine.I2C(bus)` with no pin args still drives the port default for that controller (I2C0 SDA GP4 / SCL GP5, I2C1 SDA GP6 / SCL GP7), so each side is resolved to the GPIO it actually uses before the comparison — an explicit pin resolves to itself, and because an explicit non-default routing does not reserve the unused default, a 1-Wire device on a bus's default pin is legal when that bus is explicitly routed elsewhere. This resolution is overlap-only: the same-bus identity rule above is unchanged and still treats an omitted pin as distinct from an explicit one (the port default is not config knowledge for comparing one controller's two devices' settings).

### SHT35 device

The `sht35` device is the fourth hardware device and third I2C sensor, split the same way: a low-level protocol class and a `Device` adapter.

- `SHT35` (`devices/sht35/sht35_device.py`) owns the protocol: **detection by serial-number probe**, the **command-oriented single-shot measurement cycle**, the CRC validation, and the raw-value conversion. The SHT3x-DIS is not register-mapped — a transaction is send a 16-bit command, wait, read the response — so the driver uses `writeto`/`readfrom_into`, never the `*_mem` register APIs, and the clock-stretching command set is deliberately not used: a deterministic bounded wait after a no-stretching command is portable and independent of controller clock-stretch timeout behavior. **Detection**: the SHT3x-DIS has no chip-ID register (unlike the BME280's `0xD0` or the LTR390's part nibble), so the probe is the serial-number command (`0x3682`), whose response is two CRC-protected 16-bit words — a CRC-valid answer to an SHT3x-specific command, not a mere ACK (a different device at the address answering with garbage fails the CRC and the probe moves to the next candidate). **The measurement cycle**: one single-shot command at the configured repeatability (high `0x2400` / medium `0x240B` / low `0x2416`), one bounded conversion sleep (16 / 7 / 5 ms over the datasheet maximum measurement times), a six-byte read (T MSB/LSB/CRC, RH MSB/LSB/CRC), and **both CRCs validated** — each 16-bit word is CRC-8 protected independently (polynomial `0x31`, initial `0xFF`, no reflection, no final XOR; the published vector `CRC(0xBE 0xEF) = 0x92` pins the implementation in the tests). A CRC mismatch is a bus-integrity failure and raises `OSError` like any other NACK or read failure (the operational domain) — a failed word is never reported as a reading. The raw 16-bit values are already calibrated, linearized, and compensated in the sensor, so the conversion is the published transfer function only: `T = -45 + 175·raw/65535` (°C), `RH = 100·raw/65535` (%RH). The temperature equation spans −45…+130 °C around the guaranteed −40…+125 °C measurement range and the humidity equation spans exactly 0…100, so no clamping is applied (an out-of-range reading is a real condition, not a decode artifact). It returns `(temperature_c, humidity_percent)`.
- `SHT35Device` is the `Device` adapter that applies the application-layer policy the protocol keeps out: user offsets (`offsets.{temperature_c, humidity_percent}`) applied after the conversion, to the converted channels only. Telemetry is `{temperature_c, humidity_percent}`. Derived values (dew point and the like) deliberately stay out of the driver — they are application-level calculations, not sensor channels.
- Both classes stay host-importable: `SHT35` receives an injected I2C object and `SHT35Device` imports no `machine` API, so the protocol, CRC, and conversion logic are unit-testable without hardware.

**No soft reset; known state established before it is depended on.** `init()` first normalizes, then identifies, then writes the remaining known state: for each candidate address it sends **break** (`0x3093`, with a bounded 1 ms settling wait) to stop any periodic/ART acquisition a previous user may have left — a sensor that stays powered across a controller reset, and a single-shot command does not implicitly stop periodic mode, so the known command state must exist before the probe depends on it (a NACKed break is a candidate failure, like a NACKed probe) — then binds the address with the serial-number probe, and only after successful detection writes **heater off** (`0x3066`), the ambient-measurement precondition, written explicitly rather than assumed from the power-on state. Like the LTR390 profile there is no software reset: that also makes the sequence re-runnable, so the read-failure reinit path repeats it over the held bus regardless of what a previously failed read left the sensor in.

**Bus sharing and physical-sensor uniqueness.** The I2C bus ownership is the BME280's (see there): Core 1's factory builds one `machine.I2C` per physical controller, the same-bus controller-settings rule applies, and the per-device keys are `i2c_bus` with optional `i2c_sda_pin`/`i2c_scl_pin`/`i2c_freq_hz` (the SHT3x-DIS specifies I2C up to 1 MHz, so the device cap is the RP2 controller's own ceiling). The **physical-sensor uniqueness rule covers the SHT35 with the same candidate-list semantics as the BME280**: the SHT3x-DIS offers exactly two addresses (`0x44` default, `0x45` alternate — `DEFAULT_I2C_ADDRESS_CANDIDATES = (68, 69)`, single source in `devices/sht35/validation.py`), the driver binds the first CRC-valid responder, so a single SHT35 on a bus keeps its full two-address probe list, and more than one SHT35 on a bus is legal only when each names exactly one distinct `i2c_address_candidates` address. The rule is per type: the candidate-list types never address each other's chips (BME280 `0x76`/`0x77` vs SHT35 `0x44`/`0x45`), so a `bme280` and an `sht35` on the same bus with identical controller settings are distinct physical sensors.

The device defaults to the guide's recommended environmental-telemetry profile: single-shot, **high** repeatability (membership-validated against the three SHT3x levels; the extra conversion time over low/medium is on the order of milliseconds, so high is the default unless power or throughput demonstrate a reason not to), clock stretching disabled. Each `read()` triggers one single-shot conversion and returns the sensor to idle automatically (single-shot is the low-power mode: ~0.2 µA idle vs ~600 µA measuring).

### YL-69 / FC-28 device

The `yl69_fc28` device is the fifth hardware device and the first ADC sensor, split the same way: a low-level protocol class and a `Device` adapter. The YL-69 / FC-28 is a soil **conductivity** probe — a two-electrode resistive probe behind an LM393 comparator — and the published value is the installation's **relative calibrated moisture**, never volumetric water content: dissolved salts, fertilizer, temperature, and electrode corrosion all move the conductivity, so the percentage is only comparable against this installation's own dry/wet span.

- `Yl69Fc28` (`devices/yl69_fc28/yl69_fc28_device.py`) owns the measurement cycle: **power on** (when the config switches the module's supply through an external P-MOS/load switch), a bounded **settle** sleep, **discard one sample** (the guide's post-power-up/idle first conversion is not trusted), the **median** of `sample_count` samples spaced `sample_delay_ms` apart (the median, not the mean, rejects the occasional spike a resistive probe is prone to; the odd-count constraint is validator-enforced — a median of an even count would be an arbitrary choice between two middle values), the optional **DO** read, and **power off in a `finally`**: a failed read must never leave the electrodes energized, because continuous DC through the probe is the electrolysis/corrosion mechanism the switching exists to avoid. The raw count is converted by `relative_moisture_percent(raw, dry_raw, wet_raw)` — `(raw − dry_raw) · 100 / (wet_raw − dry_raw)`, clamped to 0–100. The interpolation is **sign-agnostic**: on the common board polarity dry reads above wet (raw falls as the soil conducts), and clone boards invert it; the same formula and the same 0/100 endpoints hold either way because the calibration points were captured from the actual installation, not assumed. It returns `(raw, percent, digital)`.
- `Yl69Fc28Device` is the `Device` adapter. `initialize()` runs the shared pure validator, builds the optional Pins, and re-arms the power line off (re-runnable: the read-failure reinit path rebuilds the Pins over the same GPIOs). Telemetry is `{raw, relative_moisture_percent, digital_state}`: `raw` stays reported unmodified so a stuck ADC endpoint (a disconnected probe reads near 65535, a short near 0) stays visible next to the percentage it drives, and `digital_state` is the comparator's raw 0/1 — `null` when `digital_pin` is absent. Clone boards differ in DO polarity, so the driver reports the line as-is and leaves the interpretation (which state means "wet") to the application. There are no `offsets` for this device: the converted channel is already installation-calibrated, and there is no driver-side calibration state (the config carries it).
- Both classes stay host-importable: the ADC object is injected (Core 1 owns it, like the I2C buses and the 1-Wire bus), and the digital/power Pins are built inside `initialize()` behind a lazy `machine` import, so the cycle and the conversion are unit-testable without hardware.

**The calibration is required config.** `dry_raw` and `wet_raw` are required 16-bit counts (`read_u16()`'s 0–65535 range), captured from the installation (probe in dry soil, probe saturated), and must differ — a zero span has no meaningful scale. The fail-fast choice is deliberate: the telemetry channel `relative_moisture_percent` exists to always carry a percentage, and a raw-only mode would make the channel's meaning config-dependent. There is no driver default and no runtime recalibration command; a new installation is a new config.

**Switched power is optional and defaults to off-when-unmeasuring.** Without `power_pin` the module is assumed externally powered and the read skips the settle (nothing switched on). With it, each `read()` energizes the probe only for the settle plus sampling window — the duty cycle is roughly (200 + 9·2) ms per 20 s pass by default — which is what keeps the electrolysis current from reaching the continuous level that corrodes the electrodes. `power_active_low` (default `true`) names the polarity of the external switch (the guide's P-channel arrangement is active-low by construction); the Pin is constructed in the off state so the probe is never energized between construction and `init()`, and `init()` re-asserts it off so a previously failed read cannot leave it on.

**The read's sleeps carry operational liveness bounds.** Every sleep in the read is a single uninterrupted `sleep_ms` that does not refresh Core 1's liveness stamp, against Core 0's 30 s Core 1 staleness watchdog (the same rationale family as the `ds18b20` `conversion_ms` ceiling): `settle_ms` 0–2000 (default 200), `sample_count` odd 1–101 (default 9), `sample_delay_ms` 0–50 (default 2) — a worst-case read sleeps ≈ 2000 + 101·(50 + read) ≈ 7.1 s, well under the budget even at the ceiling.

**ADC pin ownership.** Core 1 owns its ADC channels (see Ownership invariants); a driver never creates one. `adc_pin` must be one of GP26/27/28 — the only ADC-capable GPIOs the board exposes (every other pin is rejected at the config boundary, the shared board rule in `devices/rp2_pins.py` applying to the optional `digital_pin`/`power_pin`). `core1_main` hands `DeviceManager` a factory that builds one `machine.ADC(Pin(pin))` per pin and caches it: `machine` is imported lazily inside the closure, so a config with no `yl69_fc28` device allocates no ADC and imports nothing. Unlike the I2C factory there is **no conflict raise** and the cache is a dedupe only: a pin carries no reconfigurable setting (the three channels share the one ADC peripheral and coexist), and `validate_config` already rejects two devices on one ADC pin.

**Cross-device GPIO ownership.** The config boundary enforces that one GPIO carries one signal: a soil pin (`adc_pin`, and `digital_pin`/`power_pin` when present) may not be owned by a second `yl69_fc28` device, a `ds18b20` data pin, or an I2C device's **effective** SDA/SCL (resolved to the port default when omitted, as in the 1-Wire/I2C overlap rule — an AO on another device's data line or I2C line would be flaky reads on both). Unlike the 1-Wire multidrop case, soil pins are never shared: the AO is a single-ended analog source with no bus semantics. The DO line is a plain input with **no MCU pull-up** — the FC-28 board already carries the LM393's pull-up, and an MCU pull-up could back-feed an unpowered module through DO.

**Failure domain.** Every non-`MemoryError` raise from the injected ADC and the Pins is normalized to `OSError` (the operational domain: counted, warned once, reinit at the threshold); a `MemoryError` escapes to the heap boundary. There is no chip probe and no `init()` protocol to fail — `initialize()` can only fail at the validator or the Pin construction, so a missing module degrades through the ordinary init-retry machinery like the other devices.
...
- How this boot began, as `reset_cause` (a stable short label from `machine.reset_cause()` — field-verified on the flashed build, `wdt` covers both a deliberate `machine.reset()` and a hardware-watchdog expiry, and a commanded reboot is recognizable by the `rebooting: true` acknowledgement Core 0 publishes immediately before the reset) — the breadcrumb that names the reset that preceded this boot.
...
- **The budget is derived, not arbitrary**: `WDT_TIMEOUT_MS` (8 s) sits under the RP2 hardware maximum (8388 ms, `WDT_TIMEOUT_MAX` in the rp2 port) while exceeding every single blocking operation Core 0 performs. The longest such operation is one bounded MQTT exchange wait, and both sources of those waits are bounded at the config/constant layer — `config.MAX_MQTT_BROKER_RESPONSE_TIMEOUT_SEC` (5 s, capping the CONNACK/SUBACK, PUBACK, `check_msg`-completion, and UTC-request waits) and `mqtt._MAX_PINGRESP_WAIT_SEC` (5 s) — with `tests/test_core0_wdt.py` pinning both under the watchdog budget. One *transaction* (the connect handshake, a SUBACK/PUBACK/PINGRESP wait) chains many such operations, so `MQTTClient` invokes the servicing hook — the Core 1 heartbeat check plus the watchdog feed — before every blocking socket operation: the address resolution, the TCP connect, each write, and each read (including every `wait_msg()` iteration, which is what bounds a wait loop a slow broker can stretch by trickling unrelated frames, since each individual read stays short). The longest un-fed stretch is therefore one socket operation under its own timeout. A stalled link fails on its own timeout before the watchdog can fire: a watchdog reset means "Core 0 is wedged", never "the broker was slow".
...
- **Degradation, not a reset loop — for absence only**: arming probes the *attribute* `machine.WDT` (`getattr`), never a construction catch. A build without the attribute keeps the firmware running without hardware supervision and prints a warning; making absence fatal would reboot into the same missing attribute forever (the Wi-Fi `PM_NONE` precedent). The probe is presence only because on the supported RP2 builds the capability exists: a `machine.WDT` that is *present* but fails to construct — a regression, an invalid runtime, a hardware refusal — is a real failure of a feature the board is expected to provide, and it escapes `start()` to `main.py`'s controlled-reset boundary (the `MemoryError` leg included) instead of being swallowed into a warning that leaves Core 0 without its primary supervision while everything reports healthy. A `feed()` failure, by contrast, is also a real failure and escapes to the same boundary (no catch on the feed path).
- **A reset names itself on the next boot — with one field-verified caveat**: the reset itself is silent, but `core1_main()` captures `machine.reset_cause()` once (via `SystemInformation.get_reset_cause()`, a stable short label — `wdt` for the WDT_RESET value, `poweron` for the PWRON_RESET value, else `unknown`; the v1.28 rp2 port reports no finer cause, and field-verified on the flashed build a `machine.reset()` reboot reports the WDT_RESET value, so `wdt` covers both a deliberate reset and a watchdog expiry) and the `system_startup_completed` log carries it as `reset_cause` (in both the full and the bounded fallback form), so a reset is diagnosable from the broker log rather than being only the absence of a shutdown log: a `wdt` boot with a preceding `rebooting: true` acknowledgement (Core 0 publishes it immediately before a commanded reboot) was a commanded reboot, while a `wdt` boot with no such preceding acknowledgement is unclassified within that WDT/software-reset family — a hardware-watchdog expiry or an unacknowledged commanded reset — the two not distinguishable from the field alone. The main.py recovery-boundary reset publishes no acknowledgement, so it lands in that unclassified case — a known, accepted limit (a persistent commanded-reboot marker was considered and deliberately not built); the serial `[FATAL]` line remains the recovery boundary's own trace. The get-details `machine` section reports the same field on demand, and a dropped MQTT session is attributed the same way: `Mqtt` records a stable bounded `last_disconnect_reason` (`wifi_lost` / `ping_timeout` / `publish_failure` / `check_msg_failure`) carried in the network snapshot and exposed in the get-details `communications` section.
...
- **Registry + pure entry point** (`device_factory.py`): each supported `device_type` maps to its **validation package** and its **validator/allowed-config-keys attributes**, resolved by import at first use (a type a board does not configure is never imported). `validate_device_definition()` is the pure per-device validator — generic definition shape, unknown definition fields, supported `device_type`, then dispatch to the type's pure validator. It **never constructs or initializes a hardware resource**: a valid definition with no physical backing passes, leaving physical absence to the boot-time `initialization_failed` outcome (an operational device failure, not a schema failure).
...
`config_manager.py` (Core 0-owned; the manager is passed to `Core0` and **never shared with Core 1**) owns configuration **persistence**:

- **PERSISTED vs running**: `config.json` is the committed configuration — always "what the next reboot will load" — and is what `read-config` returns. The running firmware keeps the configuration it booted with; there is no live apply, so a committed change takes effect at the next reboot. `reboot_required` is a plain in-boot flag set by the first committed change of the boot; a reboot clears it by clearing RAM (no persistent flag).
- **Classification**: a candidate semantically equal to PERSISTED is `UNCHANGED` (no filesystem write, reboot state preserved); any other changed candidate is `REBOOT_REQUIRED`. The response `changes` summary describes **this** write: PERSISTED-before vs candidate, deterministically sorted, scalar `original_value`/`new_value` pairs, and compact whole-device entries for `devices` (`change_type` `ADDED`/`REMOVED`/`MODIFIED` with `device_id` and `device_type`; a difference no id accounts for — a list-order change — is one bounded list-level `MODIFIED` entry, never the arrays).
- **File roles**: `config.json` the current committed configuration; `config.json.tmp` the fully-written uncommitted candidate; `config.json.old` the rollback journal — its presence means promotion has **not** reached the commit point (boot recovery restores it); `config.json.cleanup` the previous configuration retained after the new configuration committed — its presence means `config.json` is authoritative and the post-commit cleanup was interrupted (boot recovery releases it). Steady state after any successful boot/write is exactly `config.json`.
- **Write transaction**: `validate_config(candidate)` → load/validate PERSISTED → write `.tmp`, close, `os.sync()` → **read `.tmp` back and re-validate** (a corrupted write aborts before promotion) → `config.json`→`.old`, `.tmp`→`config.json`, `os.sync()` (the promotion is durable but still rollback-capable) → `config.json.old`→`config.json.cleanup`, `os.sync()` — **the commit point**: the synced `.old`→`.cleanup` rename is the durable record that the write committed, and `reboot_required` is set before any cleanup runs → best-effort delete `.cleanup`, `os.sync()`. A pre-commit `OSError` (a flash fault in the candidate write, or the promotion/commit rename or its sync) restores the pre-write state (`.old`→`config.json` — or `.cleanup`→`config.json` when the commit rename completed but its sync failed, which is still pre-commit-point), leaves the recovery artifacts in place when the restoration itself fails, and is reported as a `ConfigError` with code `storage_error` — a configuration-domain command failure that answers the write-config normally, never a raw `OSError` escaping to the run loop where it would be misclassified as an MQTT/network transport failure (lost acknowledgement, spurious network recovery); a corrupted candidate read-back keeps its own `unreadable_file` code and the same restoration; a post-commit cleanup failure is a logged warning only — the write is already committed and its success result is returned, and the leftover `.cleanup` stays a valid recovery copy for the next boot; `MemoryError` propagates to the fail-fast boundary at every stage, never converted into an ordinary failure.
- **Boot recovery** (`ConfigManager.recover()`, run in `main.py` before `split_config`): a present `.cleanup` means the last write committed — a valid `config.json` is the committed steady state (the leftover `.cleanup` released with any stale artifacts), and a missing/invalid `config.json` restores the valid previous config from `.cleanup` (recovery from loss of the committed configuration, not a rollback); else a valid `.old` is authoritative — its presence means a promotion was interrupted before its commit point, so it is restored even over a valid but uncommitted `config.json` (the previous committed config, stale `.tmp` removed **after** that decision); else a valid `config.json` is the committed steady state (invalid `.old`/`.cleanup` released); else a valid `.tmp` is promoted; else startup fails with `ConfigError` naming the reason — a present artifact that fails to load or validate is reported as `<path> is invalid: <its own error>`, the steady-state `config.json` first, then `.old`, then `.tmp` (a provisioned config that fails validation must not masquerade as a missing file), and only when no artifact is present at all does the message report the artifacts as all missing. Artifacts are never deleted before the recovery decision.
- **Commands**: `read-config` (payload exactly `{}`) answers `data: {config, reboot_required}` — the committed configuration, no Wi-Fi secrets. `write-config` (payload exactly `{"config": <complete candidate configuration>}` — unknown payload keys are named together as `unknown_fields`; no patch/merge/partial-update shape; `*` broadcast silently ignored) answers `data: {configuration_changed, classification, reboot_required, changes}` on success, or a bounded error carrying the `ConfigError` code (plus sorted `unknown_fields` when the cause is unknown keys, or the expected/received schema version when it mismatches). A changed write's answer is published on the currently active source/MQTT session — the candidate's values are not live until the caller issues `reboot` (write-config never reboots by itself).
...
- full dynamic configuration (the read-config/write-config subset is present since 0.4.43; every committed change requires a reboot);
