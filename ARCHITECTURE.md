...
The detailed log is still the one payload that can exceed the per-message ceiling, but only through its non-config content: device definitions now carry a count bound (`config.MAX_DEVICES`) and length bounds for `id`/`name`/`sensor_type` (`device_factory.MAX_DEVICE_*_LENGTH`), while the full `system_information` sections and driver failure reasons remain unbounded by configuration. When it exceeds the ceiling, the rejection is permanent *for that object* — Core 1 rebuilds the bounded fallback summary (the same startup statuses and device counts, but no `ready_devices`/`failed_devices` and no `system_information`), which stays far under the limit, and admits that one. A serialization `MemoryError` from the detailed message gets the same answer: on a memory-tight board (the Pico W after the device-init pass) the pool can hold tens of KiB in total yet no run large enough for the serialized form, so the size check never sees the bytes — the bounded summary is the smaller object that answers it. `core1_main()` runs an allocation-light `gc.collect()` before building the log, so a reclaimable pool admits the detailed form on the boards it fits (the Pico 2 W). The invariant this protects: failure to emit the verbose diagnostics must not keep the device from entering normal operation. Every other permanent rejection (validation, other serialization failures) still fails fast with its actual reason, and a `MemoryError` on the fallback itself propagates to the recovery boundary — nothing is admitted, nothing loops. Both the detailed message and the fallback keep the single-transient-retry admission discipline below.
