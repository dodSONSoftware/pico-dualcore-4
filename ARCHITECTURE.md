...
- **File roles**: `config.json` committed; `config.json.tmp` fully-written uncommitted candidate; `config.json.old` previous committed config — deleted as soon as the promotion succeeds, present only when a promotion was interrupted (boot recovery restores it). Steady state after any successful boot/write is exactly `config.json`.
- **Write transaction**: `validate_config(candidate)` → load/validate PERSISTED → write `.tmp`, close, `os.sync()` → **read `.tmp` back and re-validate** (a corrupted write aborts before promotion) → `config.json`→`.old`, `.tmp`→`config.json`, `os.sync()` → delete `.old`, `os.sync()`. A non-`MemoryError` failure restores the pre-write state (`.old`→`config.json`, `.tmp` released) and leaves the recovery artifacts in place; `MemoryError` propagates to the fail-fast boundary, never converted into an ordinary failure.
