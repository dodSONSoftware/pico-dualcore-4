...
**No soft reset; known state written explicitly.** `init()` runs detection, then writes every state the driver's reads depend on: **break** (`0x3093`, with a bounded 1 ms settling wait) out of any periodic/ART acquisition a previous user may have left — a single-shot command does not implicitly stop periodic mode, so an unbroken periodic state would make the single-shot reads race a running acquisition — and **heater off** (`0x3066`), the ambient-measurement precondition, written explicitly rather than assumed from the power-on state. Like the LTR390 profile there is no software reset: that also makes the sequence re-runnable, so the read-failure reinit path repeats it over the held bus regardless of what a previously failed read left the sensor in.
