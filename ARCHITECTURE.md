...
The supported `device_type` registry (`device_factory._DEVICE_REGISTRY`) maps each type to its validation package and its validator/allowed-config-keys attributes, resolved by import at first use — a type a board does not configure is never imported. Four types are registered:
