...
- How this boot began, as `reset_cause` (a stable short label from `machine.reset_cause()` — `wdt` for a hardware-watchdog reset) — the breadcrumb that names the reset that preceded this boot.
...
- **A watchdog reset names itself on the next boot**: the reset itself is silent, but `core1_main()` captures `machine.reset_cause()` once (via `SystemInformation.get_reset_cause()`, a stable short label — `wdt` for a hardware-watchdog reset, `poweron` for any other reset, else `unknown`; the v1.28 rp2 port reports no finer cause, so a `machine.reset()` reboot reports `poweron`) and the `system_startup_completed` log carries it as `reset_cause` (in both the full and the bounded fallback form), so a watchdog reset is diagnosable from the broker log instead of being only the absence of a shutdown log. The get-details `machine` section reports the same field on demand, and a dropped MQTT session is attributed the same way: `Mqtt` records a stable bounded `last_disconnect_reason` (`wifi_lost` / `ping_timeout` / `publish_failure` / `check_msg_failure`) carried in the network snapshot and exposed in the get-details `communications` section.
