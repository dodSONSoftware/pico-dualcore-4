...
- **Per-message ceiling** — `message_serializer.MAX_OUTBOUND_MESSAGE_BYTES = 16 KiB`. Bounds a single message's transient peak (graph + str + bytes ≈ 3x the payload ≈ 48 KiB) and keeps the largest legitimate messages (the startup log stream's parts — the only payloads that grow with device count) comfortably under the limit with margin. Config-derived growth is bounded at the config boundary — the device count (`config.MAX_DEVICES`) and the string fields, all measured in **UTF-8 bytes** because the ceiling is a wire bound: `source` and device `id`/`name`/`sensor_type` at 64 bytes (`MAX_SOURCE_LENGTH`, `device_factory.MAX_DEVICE_*_LENGTH`), `mqtt_broker_ip_address` at 253 bytes (`MAX_MQTT_BROKER_ADDRESS_BYTES`, the DNS hostname maximum — `socket.connect()` resolves hostnames), and the topics at 122 ASCII bytes — and a serialized-size invariant test pins that the worst-case valid configuration's read-config response, envelope splice included, stays at or under the ceiling, so a future field or driver string that breaks it fails the host suite instead of wedging read-config on a device — so the startup stream's parts are the ones that can legitimately exceed the ceiling only through non-config content (the event log's per-device lists and the `device_status` section's driver failure reasons); when one does, Core 1 answers with the bounded fallback summary for the event log and a skip for an affected part log, instead of failing startup (see Startup log). The queue enforces it on both admission paths: `put()` via the serialization step, and `put_with_kind()` via a direct byte-length check on the pre-serialized payload, so a caller bypassing `serialize_and_validate_message()` cannot admit a larger entry. The ceiling is enforced a second time at the wire boundary, against the final spliced length (`core0._publish_entry`), so the 16 KiB bound holds for the actual MQTT payload even though the envelope is added after admission.
...
Core 1 runs in a worker thread that `main.py` spawns **before Core 0's module import and the network bring-up**: the thread's default ~4 KiB stack is one contiguous allocation from the GC pool, and on the memory-tight Pico W (256 KB) no such run survives Core 0's import set plus the CYW43/lwIP buffers — a spawn that late raises `MemoryError` into `main.py`'s silent reset boundary and reboot-loops the board (the Pico 2 W's larger pool always has a run, which is why the late spawn only failed on the Pico W). The worker stamps Core 1's liveness at spawn and on every 100 ms poll, then waits for the network snapshot to report the full startup contract verified. Core 1 starts only after that contract has completed:
...
### Part logs (`system_information`) — best-effort diagnostics

One log per part, in `_STARTUP_INFORMATION_PARTS` order, immediately after the event log is admitted — seven parts instead of the nine `SYSTEM_INFORMATION_SECTIONS` entries: the `cpu`+`machine` and `devices`+`device_status` pairs are each combined into a single part whose `value` maps section name → that section's value. This is the bootup stream's grouping only; `get-details` and the config `include` list still expose all nine sections individually. `payload.data` is `{"section": <label>, "value": <part value>, "part": <i>, "parts": <n>}` (`section` is the part label — a single section name, or the comma-joined names of a combined part; 1-based); the `part`/`parts` pair makes a skipped part detectable as a gap, and the event log's `diagnostics_parts` pins the total.

Each part is built and serialized **individually** — one part value (a single section's value, or a combined pair's small `{section: value}` map) plus one serialization buffer, never the full nine-section snapshot. The full snapshot is precisely what stopped fitting on the Pico W: after the device-init pass the pool holds tens of KiB in total but no run large enough for a multi-KiB buffer, while each part serializes in a small buffer that fits. The `devices,device_status` part takes a single `get_device_sections()` walk (cached, released after the part); a non-device section comes straight from its `get_<section>()` getter.

Each part keeps the single-transient-retry discipline, but **any permanent failure skips that part with a warning and the stream continues**: an over-ceiling or validation rejection, any other serialization failure, and a serialization `MemoryError` (gc + one retry, then skip). The skip is a deliberate local `MemoryError` boundary — a deviation from the repo-wide "MemoryError propagates to the recovery boundary" contract: a best-effort diagnostic must not reboot the board (the Pico W fragmentation case is exactly a serialization `MemoryError` that a retry cannot fix), and the section's data stays retrievable on demand via `get-details`. A non-`MemoryError` getter failure still emits its part with `value[<section>] = {"error": ...}` (mirroring the collector's per-section contract), a `MemoryError` from one section's getter skips the whole combined part it belongs to, and a non-JSON-safe value is skipped (a detectable part gap). Nothing on the part path escapes to the recovery boundary: the invariant is that failure to emit the verbose diagnostics never keeps the device from entering normal operation.
...
All periodic Core 1 runtime work — telemetry and health — is scheduled from one shared epoch, `normal_runtime_start_ticks_ms`, captured exactly once, after the startup log stream completes — the `system_startup_completed` event log admitted (the gate) and the best-effort `system_information` part stream emitted. No periodic work begins before that admission. In addition, one initial sample — a telemetry read pass and a health report — is emitted at the anchor moment, immediately after admission and before the run loop, so a subscriber that connects at boot sees current data without waiting a full interval. It is a one-shot, not a second grid: the periodic boundaries below are unchanged.
...
11. **Return**: Core 0.start() returns; the ready snapshot (step 9) releases Core 1's waiting worker, which imports and starts Core 1
