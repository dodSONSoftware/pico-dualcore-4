...
| `devices/ds18b20/ds18b20_device.py` | `ds18b20` device (third hardware sensor, first 1-Wire): `DS18B20` register-free protocol class (bus scan + ROM identification, the two-stage convert/wait/read cycle with the wait releasing the 1-Wire pin for the whole window (multi-drop etiquette, ≤10 ms slices, re-acquired before the read), the −55…+125 °C range gate with the 85 °C power-on value treated as a valid temperature) and the `DS18B20Device` `Device` adapter (offset); the 1-Wire bus itself is Core 1's, built by the lazy per-pin factory in `core1.py` |
