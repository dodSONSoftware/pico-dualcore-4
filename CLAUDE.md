...
| `devices/ltr390/validation.py` | Pure `ltr390` config validation (host-importable, no `machine`): per-device I2C bus keys, gain/resolution/rate membership sets, the `measurement_rate_ms` ≥ ADC-conversion-time cross-field rule, `window_factor` ≥ 1.0, `offsets` |
...
- At boot, a one-time `system_startup_completed` startup event log is published to `mqtt_topic_log` (it carries `reset_cause`, how this boot began — `wdt` for a hardware-watchdog reset — so an unexplained reboot names itself on the next boot)
