...
| `devices/ds18b20/ds18b20_device.py` | `ds18b20` device (third hardware sensor, first 1-Wire): `DS18B20` register-free protocol class (bus scan + ROM identification, the two-stage convert/wait/read cycle with a single bounded conversion sleep — the pin stays configured but undriven: open-drain idle is high-Z, and the MicroPython ds18x20 API has no pin release — the −55…+125 °C range gate with the 85 °C power-on value treated as a valid temperature) and the `DS18B20Device` `Device` adapter (offset); the 1-Wire bus itself is Core 1's, built by the lazy per-pin factory in `core1.py` |
| `devices/ds18b20/validation.py` | Pure `ds18b20` config validation (host-importable, no `machine`): the data `pin` (any externally exposed board GPIO, the wireless-reserved GP23/24/25/29 rejected via the shared `devices/rp2_pins.py` rule), the 16-hex-char `rom` (family code `28` first, case-insensitive), `conversion_ms` 750–1000 (floor: the 12-bit maximum conversion time the driver must cover; tight ceiling: the wait is one uninterrupted sleep that does not refresh Core 1's liveness stamp, against Core 0's 30 s Core 1 staleness watchdog), `offsets` |
| `devices/sht35/sht35_device.py` | `sht35` device (fourth hardware, third I2C sensor): `SHT35` command-protocol class (no chip-ID register — detection by CRC-validated serial-number probe; single-shot no-clock-stretching measurement cycle with one bounded conversion sleep; independent per-word CRC-8, both validated; the published −45+175·raw/65535 / 100·raw/65535 transfer functions) and the `SHT35Device` `Device` adapter (offsets on the converted channels; no derived values) |
| `devices/sht35/validation.py` | Pure `sht35` config validation (host-importable, no `machine`): per-device I2C bus keys (routing via the shared `devices/rp2_i2c.py`), `i2c_address_candidates` (the SHT3x-DIS's two addresses 0x44/0x45, shared `DEFAULT_I2C_ADDRESS_CANDIDATES`), `repeatability` membership (high/medium/low, default high), `offsets` |
| `devices/yl69_fc28/yl69_fc28_device.py` | `yl69_fc28` device (fifth hardware device, first ADC sensor): `Yl69Fc28` measurement-cycle class (switched-power on → settle → discard-one → median-of-odd sampling → optional DO read → power off in `finally`; the sign-agnostic `(raw − dry_raw)·100/(wet_raw − dry_raw)` 0–100 clamped relative-moisture conversion, never VWC) and the `Yl69Fc28Device` `Device` adapter (the DO/power Pins built lazily behind a guarded `machine` import; telemetry `{raw, relative_moisture_percent, digital_state}`); the `machine.ADC` itself is Core 1's, built by the lazy per-pin factory in `core1.py` |
| `devices/yl69_fc28/validation.py` | Pure `yl69_fc28` config validation (host-importable, no `machine`): the required `adc_pin` membership in the board's three ADC-capable GPIOs (GP26/27/28), optional `digital_pin`/`power_pin` (shared `devices/rp2_pins.py` board rule) with within-device pairwise distinctness, `power_active_low` real bool (default true), the read-liveness bounds `settle_ms` 0–2000 (default 200), odd `sample_count` 1–101 (default 9), `sample_delay_ms` 0–50 (default 2) — worst-case read sleep ≈ 7.1 s against Core 0's 30 s Core 1 staleness budget — and the required `dry_raw`/`wet_raw` 16-bit calibration points that must differ |
| `devices/rp2_pins.py` | Shared pure Pico W / Pico 2 W board GPIO rule (host-importable, no `machine`): the single source of the externally exposed GPIOs (GP0–22, GP26–28) and the CYW43-reserved GP23/24/25/29 the boards do not bring out; used by the 1-Wire data-pin and I2C routing validators so a reserved pin is a config error (a "valid" config that would lose network connectivity), never an operational device failure |
| `devices/rp2_i2c.py` | Shared pure RP2 I2C pin-routing validation (host-importable, no `machine`): the single source of the rule that a pin is only valid for the SDA/SCL role of the controller its GPIO mux group belongs to (matching the RP2 port's `machine.I2C(...)` constructor check), the routable sets being the mux groups intersected with the externally exposed board pins (`devices/rp2_pins.py`); used by both I2C sensor validators so a non-routable pin is a config error, not an operational device failure |
| `config.py` | Configuration schema validation (pure `validate_config`, including device validation) and per-core splitting |
| `config_manager.py` | Core 0 configuration persistence: write transactions (`.tmp`/`.old` rollback journal, `.cleanup` post-commit artifact; the synced `.old`→`.cleanup` rename is the commit point), boot recovery; every committed change is pending a reboot (no live apply) |
...
- Recovery-timing keys carry **operational liveness bounds**: `mqtt_broker_response_timeout_sec` at 5 s (every bounded MQTT wait it sizes must fail under the 8 s watchdog budget), `network_probe_timeout_sec` at 30 s, `device_initialization_retry_delay_ms` at 60 s, `mqtt_outbound_publish_delay_ms` at 60 s (the startup publish-slot wait it paces is unsupervised, so a multi-day value must not hold startup), `device_read_failure_threshold` at 1000, `mqtt_keepalive_sec` floored at 5 s (below that, the broker-visible ping gap exceeds the broker's 1.5× tolerance), and `mqtt_command_poll_ms` capped at 10 s (the run loop pumps inbound commands only at that cadence, so a multi-day value would leave an apparently healthy device unable to service the `write-config` that would repair it)
