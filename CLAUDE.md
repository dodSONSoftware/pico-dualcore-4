...
| `devices/ds18b20/validation.py` | Pure `ds18b20` config validation (host-importable, no `machine`): the data `pin` (any of the board's 30 GPIOs), the 16-hex-char `rom` (family code `28` first, case-insensitive), `conversion_ms` 750–1000 (floor: the 12-bit maximum conversion time the driver must cover; tight ceiling: the wait is one uninterrupted sleep that does not refresh Core 1's liveness stamp, against Core 0's 30 s Core 1 staleness watchdog), `offsets` |
