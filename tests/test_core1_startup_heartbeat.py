...
def test_core1_registers_activity_stamp_before_device_initialization():
    ...
    try:
        ...
        dm.create_device = lambda device_def, i2c_bus_factory=None, onewire_bus_factory=None, adc_bus_factory=None: probe
        try:
...
def test_device_manager_normal_read_refreshes_per_device():
    ...
    saved_time = dm.time
    dm.create_device = (
        lambda device_def, i2c_bus_factory=None, onewire_bus_factory=None, adc_bus_factory=None: SlowDriver(observed)
    )
    dm.time = pass_time
    try:
